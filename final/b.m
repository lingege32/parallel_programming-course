...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    v_train_pca = v_train_t(:,1:pcaNum);
    clear v_train_t;
    clear v_train;
    pcaTrain=fopen('pcadata3.txt','w');
    pcaTrain2=fopen('pcadata2.txt','w');
    for num=1:training_num*3
        for pca=1:pcaNum
            fprintf(pcaTrain,'%f ',v_train_pca(num,pca))
    ...
    for num=1:training_num*3
        for pca=1:2
            fprintf(pcaTrain2,'%f ',v_train_pca(num,pca))
...
%%start training%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
w_level1 = ones(pcaNum+1,numOflevel2);
w_level2 = ones(numOflevel2+1,numOflevel3);  %initialize w to one
w_level3 = ones(numOflevel3+1,3);
cannot_class = 0;
for num=1:training_num
    %class 1
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    temp_z2 = sigmf(input_a2,[1 0]);
    input_z2 = [temp_z2,1];
    input_a3 = input_z2 * w_level3;
    sigma_exp = sum(exp(input_a3));
    output_y = exp(input_a3) / sigma_exp;
    ...
    diff_level3 = input_z2' * delta_k;  %3 by 3
    
    derived_h2 = sigmf(input_a2,[1 0]).*(1 - sigmf(input_a2,[1 0])); %1 by 2
    sigma_w_deltak = w_level3(1:numOflevel3,:) * delta_k';           %2 by 1
    delta_j = derived_h2 .* sigma_w_deltak';
    diff_level2 = input_z' * delta_j;
    
    derived_h = sigmf(input_a1,[1 0]).*(1 - sigmf(input_a1,[1 0])); %1 by 2
    sigma_w_deltak2 = w_level2(1:numOflevel2,:) * delta_j';           %2 by 1
    delta_m = derived_h .* sigma_w_deltak2';
    diff_level1 = input_x' * delta_m;
    
    w_level1 = w_level1 - para * diff_level1;
    w_level2 = w_level2 - para * diff_level2;
    w_level3 = w_level3 - para * diff_level3;
    %class 2
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    target = [0,1,0];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    v_test_pca = v_test_t(:,1:pcaNum);
    clear v_test_t;
    clear v_test;
    for num=1:testing_num*3
        for pca=1:pcaNum
            fprintf(pcaTrain,'%f ',v_test_pca(num,pca))
    ...
    for num=1:testing_num*3
        for pca=1:2
            fprintf(pcaTrain2,'%f ',v_test_pca(num,pca))
